...
shell.command(
  "open",
  "Connect to an SFTP over WebSockets server",
  function (context) {
...
shell.command(
  "search",
  "Search remote files (globstars allowed)",
  function (context) {
    if (!remote) return fail(context, "Not connected to a server");
...
shell.command(
  "lsearch",
  "Search local files (globstars allowed)",
  function (context) {
    var path = local.join(localPath, context.args[0]);
...
shell.command(["get", "reget"], "Download a single file", function (context) {
  ...
  var lp = context.args[1]
    ? local.join(localPath, context.args[1])
    : localPath + "/";
...
shell.command(["put", "reput"], "Upload a single file", function (context) {
  ...
  // prepare target path (if not specified, append "/" to instruct the function to use the original name
  var rp = context.args[1]
    ? remote.join(remotePath, context.args[1])
    : remotePath + "/";
...
shell.command(
  "log",
  "Turns logging on or off (only applies to new connections)",
  function (context) {
